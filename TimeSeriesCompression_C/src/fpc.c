...
1. SOFTWARE shall mean the FPC code, or portions thereof, available via the web page http://www.csl.cornell.edu/~burtscher/research/FPC/ and described in Cornell Research Foundation, Inc. (揅RF? file D-3944.  SOFTWARE includes, but is not limited to, source code, object code and executable code.
...
6. CRF may terminate this Agreement at any time if LICENSEE breaches a material provision of this Agreement.  CRF may also terminate this Agreement if the SOFTWARE becomes subject to any claim of infringement of patent, copyright or trade secret, or if in CRF抯 opinion such a claim is likely to occur.
    ...
    while (0 < intot) {
        ...
        *((long long *)&outbuf[(out >> 3) << 3]) = 0;
        for (i = 0; i < intot; i += 2) {
            ...
            // Calculate the number of leading zero bytes
            bcode = 7;                // 8 bytes
            if (0 == (xor1 >> 56))
                bcode = 6;              // 7 bytes
            if (0 == (xor1 >> 48))
