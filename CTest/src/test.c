...
{
    char *chars =
        "002 \r\n1523075453 1.77532493369E11 /r/n1523075455 1.77532529102E11";
    uint32_t
        count;
    uint64_t
        timestamp;
    ...
    assert(sscanf(chars, "%u", &count) == 1);
    printf("count: %lu\n", count);

    assert(sscanf(chars + 6, "%llu %lf", &timestamp, &value) == 2);
    printf("timestamp: %llu, value: %lf\n", timestamp, value);
}
