...
DataPoints *readUncompressedFile(
    ...
    num = fscanf(inputFile, "%llu", &count);
    assert(num == 1);
    ...
    if (timestampType == _LONG_LONG && valueType == _LONG_LONG) {
        while (cursor < count) {
            ...
            num = fscanf(inputFile, "%llu %lld", &timestamps[cursor], &values[cursor]);
            assert(num == 2);
            cursor++;
        }
    }
        ...
        while (cursor < count) {
            num = fscanf(inputFile, "%llu %lf", &timestamps[cursor], &values[cursor]);
            assert(num == 2);
            cursor++;
        }
    }
...
void writeCompressedData(
    ...
    num = fwrite(compressedDPs->metadata, sizeof(Metadata), 1, outputFile);
    assert(num == 1);

    // write the size of compressed frames
    num = fwrite(compressedDPs->tsLens, BYTES_OF_SHORT*thd, 1, outputFile);
    assert(num == 1);
    num = fwrite(compressedDPs->valLens, BYTES_OF_SHORT*thd, 1, outputFile);
    assert(num == 1);

    // compact and write the compressed data frame-by-frame
    // compact and write the compressed timestamps
    for (int i = 0; i < thd; i++) {
        num = fwrite(compressedDPs->timestamps + start, compressedDPs->tsLens[i], 1, outputFile);
        assert(num == 1);
        start += frame_b;
    }
    ...
    start = 0;
    for (int i = 0; i < thd; i++) {
        num = fwrite(compressedDPs->values + start, compressedDPs->valLens[i], 1, outputFile);
        assert(num == 1);
        start += frame_b;
    }
...
CompressedDPs *readCompressedFile(
    ...
    uint64_t
        thd, num,
        tsSize = 0, valSize = 0; // the byte size of compressed timestamps and values
    ...
    num = fread(tsLens, BYTES_OF_SHORT*thd, 1, inputFile);
    assert(num == 1);
    num = fread(valLens, BYTES_OF_SHORT*thd, 1, inputFile);
    assert(num == 1);
    ...
    assert(timestamps != NULL && values != NULL);
    num = fread(timestamps, tsSize, 1, inputFile);
    assert(num == 1);
    num = fread(values, valSize, 1, inputFile);
    assert(num == 1);
