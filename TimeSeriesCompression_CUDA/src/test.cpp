...
void test_compress_gorilla_gpu() 
{
    ...
    // compact the compressed timestamps
    compactData(compressedData_t);

    // compact the compressed values
...
void test_compress_rle_gpu()
{
    ...
    printf("%s\n", dataset);
    printf("block:%d, warp:%d, count: %lu, frame: %u\n",
        block, warp, compressedData_t->count, compressedData_t->frame);
    printStat(
        compressedData_t,
        compressionTimeMillis,
        decompressionTimeMillis
...
void test_compress_bitpack_gpu()
{
    ...
    printf("%s\n", dataset);
    printf("block:%d, warp:%d, count: %lu, frame: %u\n",
        block, warp, compressedData_v->count, compressedData_v->frame);
    printStat(
        compressedData_v,
        compressionTimeMillis,
        decompressionTimeMillis
...
void test_compress_bucket_gpu()
{
    // declare
    char inputFilePath[] = 
        "C:/Users/DELL/Desktop/TSDataset/experiment/float_value/throughput/Server43_320_b";
    char dataset[] = 
        "Server43_320_b";
    DataPoints
        *dataPoints;
